...
for i in range(int(Nx/2)):
    ...
    yuppercircle.append(5/4*radius*np.sin((i)*np.pi/(Nx/2-1)) + ymid)
...
for i in range(1,int(Nx/2)-1):
    ...
    ylowercircle.append(-5/4*radius*np.sin((i)*np.pi/(Nx/2-1)) + ymid)
...
for i in range(1,Nx-1):
    for j in range(1,Ny-1):
        xtemp = i*h
        ytemp = j*h
        if (xtemp - xmid)**(2) + 4/5*(ytemp - ymid)**(2) > radius**(2):
            xinner.append(xtemp)
            yinner.append(ytemp)
            nxinner.append('nan')
            nyinner.append('nan')
